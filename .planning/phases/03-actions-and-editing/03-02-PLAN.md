...
<task type="auto">
  <name>Task 3: Clean up Footer and wire editor to App</name>
  <files>src/components/layout/Footer.tsx, src/app.tsx</files>
  <action>
...
2. Update `src/app.tsx`:
   ...
   - Add useExternalEditor hook with context from current selection state
   - Add state for file picker visibility
   - Add `e` key handler in useInput:
     - If files.length === 0: show toast "No file to edit"
     - If files.length === 1: open directly
     - If files.length > 1: show file picker
   - Render FilePicker when picker state is active
   - On file selection: call open(selectedPath), close picker
   - File watcher will auto-refresh after edit

3. Track selected phase/todo for editor context:
   - May need to lift selection state from child components or pass it up via callbacks
   - Use existing activeTab state plus new selectedItem state
  </action>
  <verify>
    - `bun run typecheck` passes
    - `bun run lint` passes
  ...
  <done>External editor integration working with `e` key, footer cleaned up</done>
</task>
