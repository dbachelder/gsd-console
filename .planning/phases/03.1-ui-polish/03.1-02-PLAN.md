...
<task type="auto">
  ...
  <action>
Remove detail level toggle functionality from RoadmapView component:

1. Remove any 'd' key handler for detail level in RoadmapView
2. Remove any detailLevel state from RoadmapView component
3. Remove any detail level display/indicator from roadmap UI
4. Keep 'd' key reserved for future /gsd-discuss-phase command (per STATE.md)

Search RoadmapView.tsx for:
- `if (input === 'd')` - remove detail toggle handler
- `detailLevel` state - remove if exists
- Any UI showing detail level - remove from rendering
...
DO preserve all other roadmap keyboard navigation (arrows, Enter, etc.).
  ...
  <done>Detail toggle removed from roadmap view, 'd' key reserved for future /gsd-discuss-phase command</done>
</task>
