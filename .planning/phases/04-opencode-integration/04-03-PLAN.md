...
Output: Session picker UI and connect-session command.
</objective>
...
<task type="auto">
  <name>Task 3: Add connect-session command and wiring</name>
  ...
  <action>
    ...
    2. Update `src/app.tsx` to integrate SessionPicker:
       ...
       When connect-session is executed (detected in command palette):
       - Set sessionsLoading = true
       - Call getProjectSessions(process.cwd())
       - Set sessions to result
       - Set sessionsLoading = false
       - Set showSessionPicker = true

       Note: For now, we can detect connect-session by name in the palette onExecute,
       OR we can add a special "triggersSessionPicker" flag to Command type.

       Simpler approach: Make connect-session action set a ref that App checks,
       or use the existing pattern where commands just show toasts and we add
       the picker as a separate concern.

       **Recommended approach:** Add a keyboard shortcut (like 'c' for connect) that
       directly opens the session picker, similar to how 'e' opens file picker.
       This is more consistent with the existing UX.

       Add to global useInput:
       ```typescript
       if (input === 'c') {
         ...
         <Box position="absolute" marginTop={3} marginLeft={2}>
           <SessionPicker
             ...
             onSelect={(id) => {
               ...
               // For now, just show toast - actual connection in next plan
               showToast(`Selected session: ${id.slice(0, 8)}...`, 'info');
             }}
